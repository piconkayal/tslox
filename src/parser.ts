...
import { LoxError, type Token, TokenType } from "./error.js";
...
export class Parser {
  ...
  private error(token: Token, message: string): ParseError {
    if (token.type === TokenType.EOF) {
      throw new LoxError(`Error at end: ${message}`, token.line);
    } else {
      throw new LoxError(`Error at '${token.lexeme}': ${message}`, token.line);
    }
  }
